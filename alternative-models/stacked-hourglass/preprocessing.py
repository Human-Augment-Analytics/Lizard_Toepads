...
def process_images(config):
    ...
    tps_data_dir = config["tps_data_dir"]

    Path(f"{training_data_dir}/crops").mkdir(parents=True, exist_ok=True)
    Path(f"{training_data_dir}/heatmaps").mkdir(parents=True, exist_ok=True)
    ...
    for file in dir_path.iterdir():
        ...
        try:
            if ".jpg" in file.name:
                ...
                if int(imgid) > 1000:
                    process_image(imgid, model, config, tps_data_dir, training_data_dir)
            count += 1
...
def process_image(imgid, model, config, tps_data_dir, training_data_dir):
    ...
    for file in dir_path.iterdir():
        ...
        try:
            if ".jpg" in file.name:
                imgid = file.name.replace(".jpg", "")
                if int(imgid) > 1000:
                    process_image_obb(imgid, model, config, tps_data_dir, training_data_dir)
            count += 1
        except Exception as e:
            count += 1
            print()
...
def process_image_obb(imgid, model, config, tps_data_dir, training_data_dir):
    img = cv2.imread(f"{config['imgdir']}/{imgid}.jpg")
    tps = get_tps_coords(imgid, img, tps_data_dir)
    results = model(img, verbose=False)
    crops, local_tps_coords, stats, Ms = crop_toe_boxes_obb(results[0], img, tps, imgid)
    ...
    for i, crop in enumerate(crops):
        ...
        aug = base_transform(image=crop, keypoints=local_tps_coords[i].tolist())

        crop = aug["image"]
        tps_t = np.array(aug["keypoints"], dtype=np.float32)
        heatmaps = tps_to_heatmap(tps_t, crop)
        if heatmaps.shape[2] != 9:
            print()
            print(f"Mismatch found: {heatmaps.shape}")
        ...
        overlay = generate_overlay(crop, heatmaps)
        cv2.imwrite(ipath, overlay)
        np.savez(hpath, image=crop.astype(np.uint8), heatmap=heatmaps.astype(np.float32), tps=tps_t)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Preprocess images for Stacked Hourglass")
    parser.add_argument("--config", type=str, required=False, help="Name of config file in configs/")
