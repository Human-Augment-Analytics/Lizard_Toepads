"""
Run Model B training — HRNet-GCN with image-conditioned initialization on Lizard.

Trains HRNetGNNWithInit on the Lizard dataset as a validation step before
deploying to WFLW. Uses the existing Lizard split and hinit-config.json.

Orchestrates train_hinit.py via subprocess, then prints a summary table.
Follows the same conventions as alternative-models/benchmarking/run_all.py.

Usage:
    python alternative-datasets/wflw/run_hinit_lizard.py \\
        --split alternative-models/benchmarking/splits/split.json

    # Override config:
    python alternative-datasets/wflw/run_hinit_lizard.py \\
        --split alternative-models/benchmarking/splits/split.json \\
        --config alternative-models/hrnet-gcn/hinit-config.json
"""
import sys
import argparse
import logging
...
MODELS = [
    {
        "name": "hrnet_gcn_hinit",
        "dir": ALT_MODELS_DIR / "hrnet-gcn",
        "train_script": "train_hinit.py",
    },
]
...
def main():
    parser = argparse.ArgumentParser(
        description="Run HRNet-GCN (HRNet-init) training on Lizard"
    )
    parser.add_argument(
        "--split",
        type=str,
        required=True,
        help="Path to Lizard split JSON file",
    )
    parser.add_argument(
        "--config",
        type=str,
        default=DEFAULT_CONFIG,
        help=f"Path to hinit training config JSON (default: {DEFAULT_CONFIG})",
    )
    ...
    for model in MODELS:
        ...
        logging.info(f"[{name}] Starting training on Lizard...")
        start = time.time()
