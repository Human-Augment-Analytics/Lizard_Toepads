...
Used by train_wflw.py, train_hinit.py, and evaluate_wflw.py.
"""
import torch
...
def make_wflw_edge_index() -> torch.Tensor:
    ...
    Encodes the following facial regions with bidirectional edges:
    ...
    def add_chain(start, end):
        """Add bidirectional chain edges from start to end (inclusive)."""
        for i in range(start, end):
            edges.append([i, i + 1])
            edges.append([i + 1, i])

    def add_loop(start, end):
        """Add bidirectional closed-loop edges from start to end (inclusive),
        with a closing edge from end back to start."""
        for i in range(start, end):
            edges.append([i, i + 1])
            edges.append([i + 1, i])
        # close the loop
